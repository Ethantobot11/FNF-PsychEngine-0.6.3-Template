...
class DialogueCharacterEditorState extends MusicBeatState
{
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput && !animationDropDown.dropPanel.visible) {
			...
			#if mobile
			if(touchPad.buttonA.justPressed || FlxG.keys.justPressed.SPACE && UI_mainbox.selected_tab_id == 'Character') {
			...
			if(touchPad.buttonX.justPressed || FlxG.keys.justPressed.R) {
				camGame.zoom = 1;
	...
	function saveCharacter() {
		...
		if (data.length > 0)
		{
			...
			#if mobile
			StorageUtil.saveContent(characterName /*GACHA HORROR REFERANCE!?!?!?!*/ + '.json', data);
			#else
			_file = new FileReference();
